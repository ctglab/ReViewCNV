...
# Coordinates -----------------------------------
  ...
  Coordinates <- reactive({
    if (is.null(input$Bed)| is.null(input$HSLM_1)|is.null(input$FastCall_Results_1)|input$Genome == "") {
      return(NULL)}
    else{
...
# Plot for first individual -----------------------------------------------
    ...
    layout(shapes = rect, legend=list(title=list(text='Window')),
                       yaxis = list(title = 'Log2Ratio'), xaxis = list(title = "Chromosome coordinates", range = c(min=input$slider[1], 
          max=input$slider[2])))
       ...
       pl_2<- plot_ly(data = subset_data_2_range(), x = ~Position) |> 
        add_trace(type = 'scatter', mode = 'markers', y = ~Log2R, color = ~Class, colors =pal, 
         ...
         add_trace(type = 'scatter', mode = 'lines', y = ~SegMean, color = "Seg", line  = list(color = 'red'), showlegend = FALSE) |> 
          layout(shapes = rect,  yaxis = list(title = 'Log2Ratio'), xaxis = list(title = "Chromosome coordinates", range = c(min=input$slider[1], 
            max=input$slider[2])))
...
# Final plot --------------------------------------------------------------
    ...
    plt <- plt |> partial_bundle()  |>  toWebGL()
      ...
      session_store$plt
                     ...
                     input$Bed, input$HSLM_1, input$FastCall_Results_1,input$HSLM_2, input$FastCall_Results_2, 
