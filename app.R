...
Annotations_37 <- readRDS("/app/Annotations_37.rds")
Annotations_38 <- readRDS("/app/Annotations_38.rds")
  ...
  CNV3 <- reactive({
    if(!is.null(CNV2())){
     ...
     if(dim(CNV2())[1]>0){
      ...
      if(dim(CNV2())[1] >1){
          # for (i in (2:dim(CNV3)[1])){
          #   if(CNV3[i,]$n_overlap != 0){
          #     CNV3[i,]$level <-  CNV3[i,]$level +
          #       max(0,max(CNV3 |>  filter (n_overlap == 0 & Start < CNV3[1,]$End & End > CNV3[1,]$Start) |> 
          #                   select(level) ))}}
        i = 2
        while (i <= dim(CNV3)[1]){
...
# Plot for third individual -----------------------------------------------
    ...
    plt <- plt |> partial_bundle()  |>  toWebGL()
      ...
      session_store$plt
                     ...
                     input$DGV_Gold, input$GnomAD_Genome, input$GnomAD_Exome, input$Genome, input$Set_y_axis
                     )
